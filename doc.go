...
//
// Currency amounts can't be floats. Storing integer minor units (2.99 => 299) becomes
// problematic once there are multiple currencies (difficult to sort in the DB), or there
// is a need for sub-minor-unit precision (due to merchant or tax requirements, etc).
// A real arbitrary-precision decimal type is required. Since Go doesn't have one
// natively, a userspace implementation is used, provided by the cockroachdb/apd package.
// The Amount struct provides an easy to use abstraction on top of it, allowing the
...
// constructed languages (Esperanto, Volapuk), historical languages (Church Slavic, Prussian),
// secondary non-official languages (Cornish, Gujarati, Cherokee). The ignore list is maintained
// by community feedback, developers wishing to use an ignored locale can open a pull request.
//
// Once gathered, locales are filtered to remove all data not used by this package,
// and then deduplicated by parent (e.g. don't keep ``fr-CH`` if ``fr`` has the same data).
//
